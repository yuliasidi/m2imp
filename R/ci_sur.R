  ...
  if (type=='mi' & !subj_miss){
    ...
    out <- dt_sur%>%
      dplyr::mutate(p_sum = purrr::pmap(list(m2 = mean_l), bin2mi::p2_mle,
                                        dt = dt_ct))%>%
      tidyr::unnest()%>%
      dplyr::mutate(ci_u = phat_d - (1 - mean_l)*m1 + tval*sqrt(var_d + m1*sd_l^2/n_l),
                    ci_l = phat_d - (1 - mean_l)*m1 - tval*sqrt(var_d + m1*sd_l^2/n_l),
                    ni_des = ifelse(ci_u < 0, 1, 0),
                    sur = type,
                    cutv = tval)
  }
    ...
    if(type=='mi' & subj_miss){
        ...
        subj_mi_res <- subj_mi(dt_ct, num_ms = num_ms)
